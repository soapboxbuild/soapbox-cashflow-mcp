...
function runScript(script, inputJson) {
    // Use execFileSync with argument array — no shell interpolation, no injection risk
    const out = execFileSync('python3', [`${SCRIPTS_DIR}/${script}`, '--inputs', JSON.stringify(inputJson)], { timeout: 30000, encoding: 'utf-8' });
...
function createServer() {
    ...
    server.tool('run_dcf', 'Run a real estate DCF (discounted cash flow) model for an asset. Returns year-by-year NOI, exit value, unlevered IRR, and equity multiple. Supports multifamily, office, industrial, retail, and hotel.', {
    ...
    server.tool('run_intervention_irr', 'Compute the IRR and value creation impact of a single decarbonization or capex measure layered onto a base DCF model. Use this after run_dcf to screen each measure. Returns IRR, payback, NOI delta, exit value delta, and yield on cost.', {
        base_model: z.object({
    ...
    server.tool('get_ll_capture', 'Determine what fraction of energy savings flows to the landlord (LL) vs tenant (TT) for a given measure, based on lease structure, metering configuration, and BPS jurisdiction. Essential input for IRR calculation — use before run_intervention_irr.', {
    ...
    server.tool('screen_measure_portfolio', 'Screen a single decarbonization measure across multiple assets at once. For each asset, runs LL allocation + IRR calculation and returns pass/fail against the hurdle rate. Use to batch-screen one measure type (e.g. LED) across the full portfolio.', {
        ...
        return { content: [{ type: 'text', text: JSON.stringify({ summary: { total: assets.length, passed, failed: assets.length - passed }, results }, null, 2) }] };
    });
    ...
    server.tool('compute_plan_economics', 'Compute a decarbonization plan\'s value bridge and incremental IRR deterministically from per-year owner-share cash flows. Use this for EVERY decarb plan instead of hand-computing IRR / capitalization / PV. Returns the full cashflow schedule (noi_impact, unlevered, cumulative, terminal exit-value delta), the value-creation waterfall (capitalized owner savings/ancillary/fine-avoidance ÷ exit cap = exit_value_uplift, and net_value_creation = exit_value_uplift − capex + incentives), plus TWO IRRs: irr_excl_exit (operating cashflows only, NO exit residual — the pays-for-itself screen) and irr_incremental (value-inclusive, with the exit residual folded in). Supply only auditable inputs; do not pre-compute any derived field.', {
        ...
        try {
            const result = computePlanEconomics(inputs);
            return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
        }
        catch (e) {
            return { content: [{ type: 'text', text: `Error: ${e.message}` }], isError: true };
    ...
    server.tool('compute_portfolio_economics', 'Run compute_plan_economics for EVERY asset in a portfolio in ONE call and return per-asset economics + portfolio aggregates + an engine provenance stamp. Use this for the portfolio-analysis economics instead of calling compute_plan_economics per asset or hand-computing/aggregating IRR & value in code — you supply only auditable per-year owner-share flows per asset; the deterministic engine does all the money-math. Returns { provenance, portfolio:{assets_above_hurdle,total_value_creation,total_incremental_capex,...}, assets:[{asset_name,fund,irr_excl_exit,irr_incremental,net_value_creation,exit_value_uplift,above_hurdle,waterfall}] }. Report these values verbatim; never recompute them.', {
        ...
        try {
            ...
            const ownerFlowsFromMeasures = (measures) => {
            ...
            const per = assets.map((a) => {
                try {
                    const flows = (a.measures && a.measures.length) ? ownerFlowsFromMeasures(a.measures) : a.flows;
                    if (!flows || !flows.length)
                        throw new Error('asset has neither measures nor flows');
                    const r = computePlanEconomics({ flows: flows, exit_cap_rate: a.exit_cap_rate, exit_year: a.exit_year, discount_rate: a.discount_rate });
                    return {
                        asset_name: a.asset_name,
            ...
            const total_value_creation = round(ok.reduce((s, p) => s + (p.net_value_creation ?? 0), 0));
            const total_incremental_capex = round(ok.reduce((s, p) => s + (p.incremental_capex ?? 0), 0));
            const assets_above_hurdle = ok.filter((p) => p.above_hurdle).length;
            // Fund-level rollup
            const funds = {};
            for (const p of ok) {
                ...
                try {
                    trajectory = computePortfolioTrajectory({
                        ...
                        assets: assets.map((a) => ({
                            ...
                            grid_ef_annual: a.grid_ef_annual,
                            crrem_annual: a.crrem_annual,
                            exit_cap_rate: a.exit_cap_rate,
    ...
    server.tool('run_npv_payback', 'Compute simple payback and NPV for a capital measure with a flat annual savings stream — generic, no real-estate DCF model required. Use this for straightforward capex/savings measures (e.g. an energy conservation measure) that do not need the full run_dcf/run_intervention_irr real-estate model.', {
    ...
    server.tool('sum_register', 'Sum a register of measures/items by a grouping key (e.g. initiative_type) and overall. Use this for register roll-ups (category totals + grand total) instead of adding numbers by hand.', {
